...
class 每日两市成交额类( ):
	...
	def __get_day_每日两市成交额_ak(self,tradeday='20231217'):
	...
	def get_day_每日两市成交额_ak(self,tradeday):  
		#查询文件
		#爬取
		#保存
		#返回值
		...
		data_file_path=os.path.join(script_file_path,'data\\每日两市成交额_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日两市成交额_有序.json')
		...
		if(tradeday in pre_days):
			...
			temp=self.__get_day_每日两市成交额_ak(tradeday=tradeday)
			# print(temp)
			# return 
			if(temp is not None):
				#先保存后
	...
	def get_days_每日两市成交额_ak(self,start,end):
		#数据保存文件路径
		script_file_path=os.path.abspath(os.path.dirname(__file__))#当下这个代码文件路径
		data_file_path=os.path.join(script_file_path,'data\\每日两市成交额_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日两市成交额_有序.json')
	...
	def update_days_ak(self,start='20180104',end='20231220'):
		...
		for item in dys:
			print(item)
			data=s.get_days_每日两市成交额_ak(item[0],item[1])
	...
	def get_chart_html_template1(self,dynamic_data,name,iswebopen=0,src_html_path= \
		'../../../html_template1/templates/template.html',des_html_path='./static/每日两市成交额.html'):
	...
	def get_chart_html_template2(self,dynamic_data ,iswebopen=0,src_html_path= \
		'../../../html_template2/templates/template_highcharts_核密度图.html',des_html_path='./static/两市成交额_核密度图.html'):
	...
	def get_chart_每日两市成交额_核密度图(self,start='20220104',end='20231220',iswebopen=0):

		#统计数据
		data=self.get_days_每日两市成交额_ak(start=start,end=end)
		#print(data)
	...
	def get_chart_每日两市成交额(self,start='20220104',end='20231220',iswebopen=0):
		...
		左纵=0
		右纵=1
		dydata={'上证指数':[selected_index_data,左纵],'每日两市成交额':[每日_data,右纵]}
...
if __name__ == '__main__':
	...
	a=每日两市成交额类()
	# a.get_day_每日两市成交额_ak( tradeday='20231222')
	# b=a.get_days_每日两市成交额_ak( start='20231204',end='20231222')
	# print(b)
 
	a.get_chart_每日两市成交额(start='20180103',end='20231225',iswebopen=1)
	a.get_chart_每日两市成交额_核密度图(start='20190103',end='20231225',iswebopen=1)