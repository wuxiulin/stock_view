...
class 每日同花顺二级行业板块分钟数据类( ):
	def __init__(self ):
		pass
	def __get_today_同花顺二级行业板块分钟数据_wencai_js(self,blk):#没发获取历史数据只能是当天的
		...
		#url = "http://d.10jqka.com.cn/v6/time/hs_1A0001/last.js?hexin-v="上证黄白线
		...
		# 	content = driver.page_source#页面不存在可能返回不确定页面
		# 	#print(content)
	...
	def get_day_blk_同花顺二级行业板块分钟数据(self,tradeday,blk):
		...
		data_file_path=os.path.join(script_file_path,'data\\{}同花顺二级行业板块分钟数据_无序.json'.format(blk))
		data_file_path_有序=os.path.join(script_file_path,'data\\{}同花顺二级行业板块分钟数据_有序.json'.format(blk))
...
if __name__ == '__main__':
	...
	a=每日同花顺二级行业板块分钟数据类()
	out=a.get_day_blk_同花顺二级行业板块分钟数据(tradeday='20240103',blk='881175')
	print("data:   ",out)
