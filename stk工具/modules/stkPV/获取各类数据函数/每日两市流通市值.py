...
class 每日两市流通市值类( ):
	...
	def __get_day_每日两市流通市值_ak(self,tradeday='20231217'):
		try:
			df1=ak.stock_sse_deal_daily(date=tradeday)

			#print(df1)
			index_lt1= (df1.loc[df1['单日情况'] == '流通市值'].index)[0]
			#print((index_lt1),index_lt1)
			if '股票' in df1.columns:

				try:
					if(np.isnan(df1['股票'][index_lt1])):
						if(np.isnan(df1['科创板'][index_lt1])):#没有科创板数据
							temp1=int(df1['主板A'][index_lt1])+int(df1['主板B'][index_lt1])
						else:
							temp1=int(df1['主板A'][index_lt1])+int(df1['主板B'][index_lt1])+int(df1['科创板'][index_lt1])#单位是亿
					else:
						temp1=int(df1['股票'][index_lt1])#单位是亿
		...
		try:
			# 关闭特定类型的警告,这里提示函数里面有个版本问题告警，这里消除掉
			warnings.filterwarnings("ignore", category=FutureWarning, module="akshare.stock.stock_summary")
			df2=ak.stock_szse_summary(date= tradeday)
			# print(df2)
			index_lt2=(df2.loc[df2['证券类别'] == '股票'].index)[0]
			temp2=int(df2['流通市值'][index_lt2]/100000000)
		except Exception as e:
			temp2=None
			raise e

		if(temp1 is None or temp2 is None):
			print('error :'+tradeday,temp1,temp2)
			return None
		#print(temp1,temp2)
		return str(temp1+temp2)


	def get_day_每日两市流通市值_ak(self,tradeday):  
		#查询文件
		#爬取
		#保存
		#返回值
		...
		data_file_path=os.path.join(script_file_path,'data\\每日两市流通市值_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日两市流通市值_有序.json')
		...
		if(tradeday in pre_days):
			...
			temp=self.__get_day_每日两市流通市值_ak(tradeday=tradeday)
			# print(temp)
			# return 
			if(temp is not None):
				#先保存后
	...
	def get_days_每日两市流通市值_ak(self,start,end):
		#数据保存文件路径
		script_file_path=os.path.abspath(os.path.dirname(__file__))#当下这个代码文件路径
		data_file_path=os.path.join(script_file_path,'data\\每日两市流通市值_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日两市流通市值_有序.json')
		...
		for day in crawl_days:
			print(day)
			temp=self.__get_day_每日两市流通市值_ak(day)
			if(temp is not None):
	...
	def update_days_manmod(self,tradedays=['20211224'],data={}):#某天数据有误，应该是爬取保存文件有问题，就是重新爬取更新文件
		...
		if(len(tradedays)>0):
			...
			#重新爬取
			for iday in tradedays:
	 			self.get_day_每日两市流通市值_ak(tradeday=iday)
	...
	def update_days_ak(self,start='20180104',end='20231220'):
		...
		for item in dys:
			print(item)
			data=s.get_days_每日两市流通市值_ak(item[0],item[1])
	...
	def get_chart_html_template1(self,dynamic_data,name,iswebopen=0,src_html_path= \
		'../../../html_template1/templates/template.html',des_html_path='./static/每日两市流通市值.html'):
	...
	def get_chart_html_template2(self,dynamic_data ,iswebopen=0,src_html_path= \
		'../../../html_template2/templates/template_highcharts_核密度图.html',des_html_path='./static/每日两市流通市值_核密度图.html'):
	...
	def get_chart_每日两市流通市值_核密度图(self,start='20220104',end='20231220',iswebopen=0):

		#统计数据
		data=self.get_days_每日两市流通市值_ak(start=start,end=end)
		#print(data)
	...
	def get_chart_每日两市流通市值(self,start='20220104',end='20231220',iswebopen=0):
		...
		左纵=0
		右纵=1
		dydata={'上证指数':[selected_index_data,左纵],'每日两市流通市值':[每日_data,右纵]}
...
if __name__ == '__main__':
	...
	a=每日两市流通市值类()
	#c=a.get_day_每日两市流通市值_ak( tradeday='20210302')
	#print(c)
	#a.update_days_manmod(tradedays=['20190605','20191113' ],data={})
	#c=a.get_day_每日两市流通市值_ak( tradeday='20211224')
	#print(c)

	#b=a.get_days_每日两市流通市值_ak( start='20190103',end='20231222')
	#print(b)
 
	a.get_chart_每日两市流通市值(start='20180103',end='20231222',iswebopen=1)
