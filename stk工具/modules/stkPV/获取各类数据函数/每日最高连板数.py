...
class 每日最高连板数类():
	...
	def __get_day_每日最高连板数_wencai(self,tradeday='20231217'):		
		#通过问财爬取，历史数据，然后保存到熬本地
		searchtxt = tradeday+'连续涨停天数,去掉st'
		try:
		...
		if(res is None or len(res)==0):
			print(e,"get_day_每日最高连板数   is  none ") 
			return  None
	 	
		col=[ item for item in res.columns if "连续涨停天数" in item]
		if(len(col)!=1):
			print(col,'列名字有问题，连续涨停天数(天)')
			return None
		else:
			col=col[0]
		#这里搜索结果默认是排序的，所以直接用结果，以后如果发现问题在排序后取出

		return str(res[col][0])

	def get_day_每日最高连板数_wencai(self,tradeday):  
		#查询文件
		#爬取
		#保存
		#返回值
		...
		data_file_path=os.path.join(script_file_path,'data\\每日最高连板数_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日最高连板数_有序.json')
		...
		if(tradeday in pre_days):
			...
			temp=self.__get_day_每日最高连板数_wencai(tradeday=tradeday)
			#print(temp)
			if(temp is not None):
				#先保存后
	...
	def get_days_每日最高连板数_wencai(self,start,end):
		#数据保存文件路径
		script_file_path=os.path.abspath(os.path.dirname(__file__))#当下这个代码文件路径
		data_file_path=os.path.join(script_file_path,'data\\每日最高连板数_无序.json')
		data_file_path_有序=os.path.join(script_file_path,'data\\每日最高连板数_有序.json')
		...
		for day in crawl_days:
			print(day)
			temp=self.__get_day_每日最高连板数_wencai(day)
	...
	def get_day_每日最高连板数_龙头股_wencai(self,tradeday):  #有几个股是最高空间板
		...
		high板=res[col][0]
		stocks=[]
		for i in range(10):#10个超过不可能很小
			if(res[col][i] ==high板 ):
				stocks.append(res['code'][i] )
	...
	def update_days_manmod(self,tradedays=['20211224'],data={}):#某天数据有误，应该是爬取保存文件有问题，就是重新爬取更新文件
		...
		if(len(tradedays)>0):
			...
			#重新爬取
			for iday in tradedays:
	 			self.get_day_每日最高连板数_wencai(tradeday=iday)
	...
	def update_days_wencai(self,start='20180104',end='20231220'):
		...
		for item in dys:
			print(item)
			data=s.get_days_每日最高连板数_wencai(item[0],item[1])
	...
	def get_chart_html_template1(self,dynamic_data,name,iswebopen=0,src_html_path= \
		'../../../html_template1/templates/template.html',des_html_path='./static/每日最高连板数.html'):
	...
	def get_chart_每日最高连板数(self,start='20220104',end='20231220',iswebopen=0):
		...
		#统计数据
		data=self.get_days_每日最高连板数_wencai(start=start,end=end)
		#print(data)
		...
		左纵=0
		右纵=1
		dydata={'上证指数':[selected_index_data,左纵],'每日最高连板数':[每日_data,右纵]}
...
if __name__ == '__main__':
	...
	a=每日最高连板数类()
	#b=a.get_day_每日最高连板数_wencai( tradeday='20231221')
	#print(b)
	#d=a.get_days_每日最高连板数_wencai( start='20231211',end='20231220')
	#print(d)
	a.get_chart_每日最高连板数(start='20220613',end='20231219',iswebopen=1)
