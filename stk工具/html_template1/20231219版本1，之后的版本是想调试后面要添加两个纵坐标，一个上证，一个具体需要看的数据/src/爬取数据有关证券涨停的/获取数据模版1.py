...
import os
import time
def get_days_wencai(infile,start,end,txt):
	...
	# 指定保存的文件路径
	oldname='old_'+os.path.basename(file_path)
	old_path=os.path.join( os.path.dirname(file_path),oldname)
 
	if os.path.exists(file_path):
		if os.path.exists(old_path):
			os.remove(old_path)
			os.rename(file_path, old_path)#备份
		else:
			os.rename(file_path, old_path)#备份
...
if __name__ == '__main__':
	...
	for i in tt:
		print(i)
		try:
			get_days_wencai(start=i[0],end=i[1],txt="涨停,证券板块",infile='测试.json')#测试保存在本文件夹就好了
