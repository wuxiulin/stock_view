...
class Weibo(object):
    ...
    def get_article_url(self, selector):
        ...
        if text.startswith("发布了头条文章"):
            ...
            url = selector.xpath("//a/@href")#wxl20240110,这里似乎换代码了
            #if url and url[0].startswith("http://t.cn"):
            #    article_url = url[0]
            article_url=url
            pic_url=selector.xpath("//img/@src")#头条文章的封面图片
        #print(url)
                    ...
                    if w["card_type"] == 9:
                        ...
                        if wb:
                            ...
                            if (not self.only_crawl_original) or ("retweet" not in wb.keys()):
                                ...
                                # 这里是系统日志输出，尽量别太杂
                                logger.info(
                                    "已获取用户 {} 的微博，简要内容为 {}".format(
                                        self.user["screen_name"],(wb["text"][:20] if len(wb["text"]) > 10 else wb["text"])#简要打印
                                    )
                                )
                                # self.print_weibo(wb)
                            else:
    ...
    def fromat_html_txt(self,html_string):
        modified_html = re.sub(r'</title>.*?</head>', '</title></head>', html_string, flags=re.DOTALL)
        modified_html = re.sub(r'<body class="B_artical ">.*?<div class="main_editor',r'<body class="B_artical ">.*?<div class="main_editor', modified_html, flags=re.DOTALL)
        modified_html = re.sub(r'<div node-type="thirdModule">.*?</body>',r'<div node-type="thirdModule"></body>', modified_html, flags=re.DOTALL)
        modified_html=modified_html.replace('style="visibility: hidden"',"")
    ...
    def get_headline_article(self):#根据获取的url，获取头条文章
        #为了不跟原来代码混在一起，这里是在爬取完一个用户后写入到csv中后，读取然后爬取
        print('抓取头条文章')

        # 读取CSV文件
        csv_file_path = self.get_filepath("csv")  # 替换为你的CSV文件路径
        #print(csv_file_path)

        if os.path.exists(csv_file_path):
            df = pd.read_csv(csv_file_path,index_col=None)
        else:
            print(self.user_csv_file_path ,'，文件不存在，需要更新')
            return

        # #获取已经爬取的时间，这里根据user_id_list.txt设置的，如果没有早就报错了，没有时间设置，这里应该也是有问题，返回告警
        # usr_list={}#特殊处理一下
        # with open('user_id_list.txt', 'r', encoding='utf-8') as file:
        #     # 逐行读取文件内容
        #     for line in file:
        #         # 使用空格分割每行的字段
        #         user_info = line.strip().split(' ')
        #         # user_info 现在是一个包含不同字段的列表
        #         # 你可以按需使用这些字段
        #         user_id = user_info[0]
        #         username = user_info[1]
        #         timestamp = user_info[2]
        #         usr_list[user_id]=timestamp

        # 遍历CSV中的URL列并爬取内容
        templasttime= datetime.strptime('2100-01-11T21:57:06', "%Y-%m-%dT%H:%M:%S")

        for index, url in enumerate(df['头条文章url'][::-1]):
            #csv是追加方式写的，但是设置时间或手动修改时间文件死后，可能又重复项目，但是如果不修改，不乱该配置，没有问题时候
            #这里增加时间判断还有厎是头条文章爬取url，这里是在原有代码执行完成基础上爬取，配合csv文件处理，防止全部爬取、重复爬取
            #爬取所有最新一次更新的，可能还是有已经保存本地的，无所谓，不在修改了太麻烦，其实不用
            if(datetime.strptime(df["日期"][index], "%Y-%m-%dT%H:%M:%S") < templasttime):#从user_id_list.txt获取最新更新的截止时间
                break
            ...
            if response.status_code == 200:
                ...
                txt='''$CONFIG['isMask'] = '0'''
                if(txt) in html_content:#收费：
                    continue 
                # 保存到HTML文件
                output_file_path = os.path.dirname(csv_file_path)+'\\'+f'{str(self.user_config["user_id"])}头条文章_{df["日期"][index]}.html'.replace(':','_')
                
                html_content=self.fromat_html_txt(html_content)
                with open(output_file_path, 'w', encoding='utf-8') as file:
                    file.write(html_content)
                print(f'HTML content from {url[0]} saved to {output_file_path}')
            else:
                print(f'Failed to fetch content from {url[0]}. Status code: {response.status_code}')
            templasttime= datetime.strptime(df["日期"][index], "%Y-%m-%dT%H:%M:%S")
            sleep(random.randint(6, 10))
    ...
    def start(self):
        ...
        try:
            for user_config in self.user_config_list:
                ...
                #这里补充头条文章的爬取工作wxl20240110
                self.get_headline_article()
                logger.info("*" * 100)
        except Exception as e:
            logger.exception(e)
...
if __name__ == "__main__":
    #重新爬取要清user_id_list.txt
    #爬取是从user_id_list.txt设置的时间开始，没有设置时间，就从cofig.json设置的时间开始 "since_date": "2022-01-07"
    #如果都设置好反复执行带代码，不会出错
    #但是如果"since_date"不改，而是设置user_id_list.txt设置的时间开始，不是清空，会导致xxxxx.csv重复，这里是手动操作引发的
    #这时候清除这个xxx.csv就好了,爬取就冲user_id_list.txt设置的时间开始，
